 ...
 */
public class TrendBigPatternView extends View {
    ...
    public TrendBigPatternView(Context context) {
        super(context);
    }

    public TrendBigPatternView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TrendBigPatternView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            for (int i = 0; i < currPattern.length; i++) {//中间矩形和文字
                ...
                //再画文字
                float centerX = (left + right) /2;
                canvas.drawText(MapUtils.BIG_EVEN[b],(int) centerX,(int) (size/2 - fontTop/2 - fontBottom/2), PaintUtils.SMALL_WHITE_TEXT);//最后画圆内的文字
            }
        } catch (Exception e) {
            e.printStackTrace();
