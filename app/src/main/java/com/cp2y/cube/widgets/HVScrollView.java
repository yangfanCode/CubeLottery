        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    final int activePointerIndex = ev.findPointerIndex(mActivePointerId);
                    final float y = ev.getY(activePointerIndex);
                    final float deltaY = startY - y;
                    final float x = ev.getX(activePointerIndex);
                    final float deltaX = startX - x;
                    if(mFlingEnabled){
                        final VelocityTracker velocityTracker = mVelocityTracker;
                        velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
                        int initialVelocitx = (int) velocityTracker.getXVelocity();
                        int initialVelocity = (int) velocityTracker.getYVelocity();
                        Log.d("Scroll", "deltaX:" + deltaX +",deltaY"+deltaY+",Vx:"+initialVelocitx+",Vy:"+ initialVelocity);
                        if (getChildCount() > 0 && Math.abs(deltaY) > 2) {
