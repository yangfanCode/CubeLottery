...
import com.cp2y.cube.fragment.chongqing2.adapter.MyFilterCQ2ZhiXuanAdapter;
 ...
 */
public class CQ2ZhiXuanFragment extends FilterBaseFragment implements BallSelectCall, MyInterface.setSelectNumPopUps, MyInterface.isSavaNumSuccess {
    ...
    private MyGridView  gv_shi, gv_ge;
    private LinearLayout tip_ll;
    private int selectCount;
    private MyFilterCQ2ZhiXuanAdapter adapter_shi, adapter_ge;
    ...
    private TextView tv_shi, tv_ge, tv_count;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_cq2_zhi_xuan, container, false);
    ...
    private void initListener() {
        ...
        btn_save.setOnClickListener((v -> {
            ...
            if (loginState) {
                if (list_select_shi.size() > 0 && list_select_ge.size() > 0) {
                    if (list_select_shi.size() == 1 && list_select_ge.size() == 1) {//单式
                        //判断相同号码
                        list_check1.clear();
                        list_check1.addAll(list_select_shi);
                        list_check1.addAll(list_select_ge);
                        if (list.size() > 0) {//保存过
                            if (list_check1.get(0).equals(list.get(0)) && list_check1.get(1).equals(list.get(1)) ) {//判断重复
                        ...
                        getService(CQ2LotteryService.class).saveLotteryNumber(list, 0, 7,selectCount);
                    } else {//定位
                        //判断相同号码
                        list_check2.clear();
                        list_check2.addAll(list_select_shi);
                        //检验蓝球特殊标记 加50
                        ...
                        getService(CQ2LotteryService.class).saveLotteryNumber(list, 1, 7,selectCount);
                    }
                } else {
    ...
    private void initData() {
        ...
        adapter_shi = new MyFilterCQ2ZhiXuanAdapter(getActivity(), false, list_select_shi, 0);
        gv_shi.setAdapter(adapter_shi);
        adapter_shi.LoadData(list_num);
        adapter_ge = new MyFilterCQ2ZhiXuanAdapter(getActivity(), false, list_select_ge, 1);
    ...
    //下方选号提示
    public void ShowTip() {
        boolean showTips = list_select_shi.size() > 0 && list_select_ge.size() > 0;
        ViewUtils.showViewsVisible(showTips, tip_ll);
        if (!showTips) {
            selectCount = 0;
        } else {
            selectCount = (list_select_shi.size() * list_select_ge.size());
    ...
    public void editData(List<String> list) {
        clear();//清除数据
        if (list.size() == 2) {//直选单式
            list_select_shi.add(list.get(0));
            list_select_ge.add(list.get(1));
        } else {//直选定位
            for (int i = 0; i < list.size(); i++) {
                if (Integer.parseInt(list.get(i)) < 50) {
                    list_select_shi.add(list.get(i));
                } else if (Integer.parseInt(list.get(i)) >= 50 && Integer.parseInt(list.get(i)) < 100) {
                    list_select_ge.add((Integer.parseInt(list.get(i)) - 50) + "");
                }
            }
        }
        adapterNotify();//刷新
    ...
    public void submit_filter(int type) {
        ...
        if (SingletonMap3D.getMapSize() >= 10 && isEdit == false) {
            ...
            if (list_select_shi.size() == 1 && list_select_ge.size() == 1) {
                //直选单式 不加标记
                listfilter_zhixuan.addAll(list_select_shi);//添加十位
                listfilter_zhixuan.addAll(list_select_ge);//添加个位
            } else {
                //直选定位
                listfilter_zhixuan.addAll(list_select_shi);//添加百位
                //个位特殊标记 加50
                for (String num : list_select_ge) {
                    //添加个位
