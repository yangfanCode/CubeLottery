...
import com.cp2y.cube.adapter.MyFilterNomaiBlueBallAdapter;
import com.cp2y.cube.adapter.MyFilterNomaiRedBallAdapter;
 ...
 */
public class NomalFragment extends FilterBaseFragment implements  BallSelectCall,MyInterface.setSelectNumD3PopUps,MyInterface.isSavaNumSuccess{
    ...
    private static MyFilterNomaiRedBallAdapter adapter_red;
    private static MyFilterNomaiBlueBallAdapter adapter_blue;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        adapter_red=new MyFilterNomaiRedBallAdapter(getActivity(),R.layout.item_selectfilter_nomalred,ignore,list_red_select);
        gv_red.setAdapter(adapter_red);
        adapter_red.initData(list_red);
        for (int i = 1; i < 17; i++) {
            list_blue.add(i+"");
        }
        adapter_blue=new MyFilterNomaiBlueBallAdapter(getActivity(),R.layout.item_selectfilter_nomalblue,ignore,list_blue_select);
            ...
            if(loginState) {
                if (list_red_select.size() >= 6 && list_blue_select.size() > 0) {
                    if (list_red_select.size() + list_blue_select.size() == 7) {
                        ...
                        list.addAll(list_blue_select);
                        getService(LotteryService.class).saveLotteryNumber(list, 0, 0,selectCount);
                        ...
                        getService(LotteryService.class).saveLotteryNumber(list, 1, 0,selectCount);
                    }
                } else {
                    TipsToast.showTips("请至少选择6个红球+1个蓝球");
                }
            }else{
        ...
        btn_cancel.setOnClickListener(view -> ContextHelper.getActivity(SelectNumFilterActivity.class).gotoConditionPage());
        btn_back.setOnClickListener(view -> ContextHelper.getActivity(SelectNumFilterActivity.class).gotoConditionPage());
    ...
    private void numSet(int pos1,int pos2) {
        SelectNumFilterActivity act =  ContextHelper.getActivity(SelectNumFilterActivity.class);
    ...
    public void shark(){
        List<String>list_red_edit=new ArrayList<>();
        List<String>list_blue_edit=new ArrayList<>();
        List<Integer> list=CombineAlgorithm.RandomNum();
        for (int i = 0; i < list.size(); i++) {
            list_red_edit.add(list.get(i)+"");
        }
        list_blue_edit.add(((int)(Math.random()*16)+1)+"");
        //排序红球
    ...
    public void submit_filter(int type){
        ...
        if(SingletonMapNomal.getMapSize()>=10&&isEdit==false){
            ...
            ContextHelper.getActivity(SelectNumFilterActivity.class).gotoConditionPage();
        }else{
