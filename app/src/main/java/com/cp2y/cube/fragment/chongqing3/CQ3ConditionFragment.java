 ...
 */
public class CQ3ConditionFragment extends Fragment implements HistoryDialog.HistoryCall, DoubleNumDialog.DoubleNumCall, SumDialog.SumCall, SpanDialog.SpanCall, D3BigDialog.BigCall, D3OddDialog.OddCall, DivideDialog.DivideCall ,SumMantissaDialog.MantissaCall{
    ...
    private void loadConditions() {
        if (SingletonMapFilter.getCQ3Map().size() > 0) {
            myFilter_layout.setVisibility(View.VISIBLE);
            for (Iterator<Map.Entry<String, List<String>>> it = SingletonMapFilter.getCQ3Map().entrySet().iterator(); it.hasNext(); ) {
    ...
    private void initData() {
        ...
        historyDialog.setFlag(5);//setFlag为公用的弹窗
        ...
        listView.setOnItemClickListener((adapterView, view1, i, l) -> {//跳转编辑
            ...
            ContextHelper.getActivity(SelectCQ3XingNumActivity.class).editData(list, entry.getKey(), keyNum);
        });
    ...
    private void initListener() {
        ...
        findViewById(R.id.filter_condition_ivAddNum).setOnClickListener(view -> {
            if (lotteryAdapter.getCount() < 10) {
                ContextHelper.getActivity(SelectCQ3XingNumActivity.class).setAddMode();
            } else {
