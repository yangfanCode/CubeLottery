...
import com.cp2y.cube.widgets.trend.TrendBigPatternView;
 ...
 */
public class TrendBigPatternAdapter extends TrendSimpleAdapter implements TrendAdapter<IssueAnalyseDataModel.IssueAnalyse> {
    private int flag=-1;
    ...
    public TrendBigPatternAdapter(Context context, ConditionModel condition, int flag) {
    ...
    public void initNums() {
        ...
        for (int i = 0; i < getCount(); i++) {
            ...
            for (int j = 0; j < num.length; j++) {
                num[j] = MapUtils.BIG_EVEN_MAP.get(pattern.substring(j,j+1)).byteValue();
            }
            nums.add(num);
        }
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        if (convertView == null) {
            if(flag==4){
                convertView = LayoutInflater.from(context).inflate(R.layout.item_trend_p5_big_pattern, null);
            }else{
                convertView = LayoutInflater.from(context).inflate(R.layout.item_trend_big_pattern, null);
            }
        }
        try {
...
//        ViewUtils.showViewsVisible(condition.isCalcBlue(),converew.findViewById(R.id.trend_line3));
            TrendBigPatternView bigPatternView = (TrendBigPatternView) convertView.findViewById(R.id.trend_big_pattern);
            bigPatternView.setFlag(flag);
            bigPatternView.setCalcBlue(condition.isCalcBlue());
            bigPatternView.setPatterns(currNum, prevNum, nextNum);
            TextView text = (TextView) convertView.findViewById(R.id.trend_big_text);
            StringBuilder sb = new StringBuilder();
            for (byte b:currNum) {
                sb.append(MapUtils.BIG_EVEN[b]);
            }
