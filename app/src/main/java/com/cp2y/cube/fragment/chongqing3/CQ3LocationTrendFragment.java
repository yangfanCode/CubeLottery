 ...
 * 定位走势
 */
public class CQ3LocationTrendFragment extends TrendD3BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.fragment_cq3_location_trend, container, false);
...
//        });
    ...
    protected void reloadData() {
        ...
        if (condition.getPeriod() != condition.getCurrentPeriod() || condition.getWeekDay() != condition.getCurrentWeekDay()) {//加载期数不相同时再加载
            NetHelper.LOTTERY_API.getIssueNumber(10093, "10002", condition.getPeriod(), condition.getWeekDay(), 1)
    ...
    public void setListener(int pos){
        if(pos==0){
            setSubmitChangeListener(pos,list_check_bai,adapter_bai);
        }else if(pos==1){
            ...
            list.get(i).setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        ...
                        if(pos==0){
                            list_bai_select.add(String.valueOf(finalI));
                            num_bai++;
                        }else if(pos==1){
                        ...
                        if(pos==0){
                            list_bai_select.remove(String.valueOf(finalI));
                            num_bai--;
                        }else if(pos==1){
                    ...
                    //排序
                    CommonUtil.SortCollection(pos==0?list_bai_select:(pos==1?list_shi_select:list_ge_select));
                    //第一次点击就显示
