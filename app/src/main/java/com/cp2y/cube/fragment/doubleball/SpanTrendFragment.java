 ...
 * 跨幅度走势
 */
public class SpanTrendFragment extends TrendBaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.fragment_span_trend, container, false);
        initSettingDialog(getString(R.string.span_tips), MapUtils.SPAN_SUPPORT, new int[]{1,2},0);
        //默认不显示开奖号
        ...
        dataAdapter = new TrendRangeAdapter(getContext(),condition,0);
...
//        });
    ...
    private void initListener() {
        ...
        tv_submit.setOnClickListener((v)->{
            ...
            if(list_data!=null&&list_data.size()>0){
                list_flag.clear();
                list_flag.addAll(list_data);
                SingletonMapFilter.registerService("4", list_data);
            }
    ...
    private void resizeLayout() {
        ...
        relativeLayout.setHsvViewSize(DisplayUtil.dip2px(condition.isShowBaseNum()?1002f:846f), DisplayUtil.dip2px(30.1f * dataAdapter.getCount()-1));//默认设置450dp
    }
    ...
    protected void reloadData() {
        ...
        if (condition.getPeriod() != condition.getCurrentPeriod() || condition.getWeekDay() != condition.getCurrentWeekDay()) {//加载期数不相同时再加载
            NetHelper.LOTTERY_API.getIssueAnalyse(10002, "10013", condition.getPeriod(), condition.getWeekDay())
