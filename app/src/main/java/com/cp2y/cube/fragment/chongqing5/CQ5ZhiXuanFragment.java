...
import com.cp2y.cube.fragment.chongqing5.adapter.MyFilterCQ5ZhiXuanAdapter;
 ...
 */
public class CQ5ZhiXuanFragment extends FilterBaseFragment implements BallSelectCall, MyInterface.setSelectNumPopUps, MyInterface.isSavaNumSuccess {
    ...
    private MyFilterCQ5ZhiXuanAdapter adapter_wan,adapter_qian,adapter_bai, adapter_shi, adapter_ge;
    ...
    private void initListener() {
        ...
        btn_cancel.setOnClickListener((v -> ContextHelper.getActivity(SelectCQ5NumActivity.class).gotoConditionPage()));
        btn_back.setOnClickListener((v -> ContextHelper.getActivity(SelectCQ5NumActivity.class).gotoConditionPage()));
            ...
            if (loginState) {
                if (list_select_wan.size() > 0 && list_select_qian.size() > 0 &&list_select_bai.size() > 0 && list_select_shi.size() > 0 && list_select_ge.size() > 0) {
                    if (list_select_wan.size() == 1 &&list_select_qian.size() == 1 &&list_select_bai.size() == 1 && list_select_shi.size() == 1 && list_select_ge.size() == 1) {//单式
                        ...
                        getService(P5LotteryService.class).saveLotteryNumber(list,0,6,selectCount);
                    } else {//定位
                        //判断相同号码
                        list_check2.clear();
                        ...
                        getService(P5LotteryService.class).saveLotteryNumber(list, 1,6,selectCount);
                    }
                } else {
    ...
    private void initData() {
        ...
        adapter_wan = new MyFilterCQ5ZhiXuanAdapter(getActivity(), false, list_select_wan, 0);
        gv_wan.setAdapter(adapter_wan);
        adapter_wan.LoadData(list_num);
        adapter_qian = new MyFilterCQ5ZhiXuanAdapter(getActivity(), false, list_select_qian, 1);
        gv_qian.setAdapter(adapter_qian);
        adapter_qian.LoadData(list_num);
        adapter_bai = new MyFilterCQ5ZhiXuanAdapter(getActivity(), false, list_select_bai, 2);
        gv_bai.setAdapter(adapter_bai);
        adapter_bai.LoadData(list_num);
        adapter_shi = new MyFilterCQ5ZhiXuanAdapter(getActivity(), false, list_select_shi, 3);
        gv_shi.setAdapter(adapter_shi);
        adapter_shi.LoadData(list_num);
        adapter_ge = new MyFilterCQ5ZhiXuanAdapter(getActivity(), false, list_select_ge, 4);
    ...
    private void numSet(int pos1, int pos2) {
        SelectCQ5NumActivity act = ContextHelper.getActivity(SelectCQ5NumActivity.class);
    ...
    public void submit_filter(int type) {
        ...
        if (SingletonMapP5.getMapSize() >= 10 && isEdit == false) {
            ...
            ContextHelper.getActivity(SelectCQ5NumActivity.class).gotoConditionPage();
        } else {
    ...
    public void setSelectNumPopUp(int x, int y, int position, boolean isShow) {
        if(isShow){
            if(selectNum_popUp.getChildCount()==0){
                ...
                Toolbar toolbar=ContextHelper.getActivity(SelectCQ5NumActivity.class).getToolBar();
                toolbar.measure(0,0);
