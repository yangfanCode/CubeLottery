 ...
 * 定位走势
 */
public class CQ2LocationTrendFragment extends TrendD3BaseFragment {
    ...
    private List<String>list_shi_select,list_ge_select;
    private ArrayAdapter<String> adapter_shi,adapter_ge;
    ...
    private String[] tabTitle={"十位","个位"};
    private TextView submit_tv,submit_tv_tip;
    private HScrollView shi_ll,ge_ll;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rootView = inflater.inflate(R.layout.fragment_cq2_location_trend, container, false);
...
//        });
    ...
    private void initView(View rootView) {
        ...
        LinearLayout ll= (LinearLayout) shi_ll.findViewById(R.id.location_trend_ll);
    ...
    protected void reloadData() {
        ...
        if (condition.getPeriod() != condition.getCurrentPeriod() || condition.getWeekDay() != condition.getCurrentWeekDay()) {//加载期数不相同时再加载
            NetHelper.LOTTERY_API.getIssueNumber(10095, "10002", condition.getPeriod(), condition.getWeekDay(), 1)
    ...
    public void onDoubleTaped() {
        ...
        closePop();
        //双击关闭选号提示
        if(list_shi_select.size()>0||list_ge_select.size()>0) {
    ...
    public void setListener(int pos){
        if(pos==0){
            setSubmitChangeListener(pos,list_check_shi,adapter_shi);
        }else{
            setSubmitChangeListener(pos,list_check_ge,adapter_ge);
        }
    }
            ...
            list.get(i).setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        ...
                        if(pos==0){
                            list_shi_select.add(String.valueOf(finalI));
                            num_shi++;
                        }else{
                            list_ge_select.add(String.valueOf(finalI));
                            num_ge++;
                        }
                    } else {
                        if(pos==0){
                            list_shi_select.remove(String.valueOf(finalI));
                            num_shi--;
                        }else{
                            list_ge_select.remove(String.valueOf(finalI));
                            num_ge--;
                        }
                    }
                    //控制开关和 弹窗
                    if (num_shi > 0 && num_ge > 0) {
                        tv_sunbmit.setEnabled(true);
                        submit_tv.setText("保存");
                        submit_tv_tip.setVisibility(View.GONE);
                    } else if (num_shi == 0 && num_ge == 0) {
                    ...
                    //排序
                    CommonUtil.SortCollection(pos==0?list_shi_select:list_ge_select);
                    //第一次点击就显示
