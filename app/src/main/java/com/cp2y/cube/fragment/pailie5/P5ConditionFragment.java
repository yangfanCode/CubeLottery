 ...
 */
public class P5ConditionFragment extends Fragment implements HistoryDialog.HistoryCall, SumDialog.SumCall, SpanDialog.SpanCall, BigDialog.BigCall, OddDialog.OddCall, DivideDialog.DivideCall, SumMantissaDialog.MantissaCall {
    ...
    private void loadConditions() {
        if (SingletonMapFilter.getP5Map().size() > 0) {
            myFilter_layout.setVisibility(View.VISIBLE);
            for (Iterator<Map.Entry<String, List<String>>> it = SingletonMapFilter.getP5Map().entrySet().iterator(); it.hasNext(); ) {
    ...
    private void initData() {
        ...
        listView.setOnItemClickListener((adapterView, view1, i, l) -> {//跳转编辑
            ...
            ContextHelper.getActivity(SelectP5NumActivity.class).editData(list, entry.getKey());
        });
    ...
    private void initListener() {
        ...
        findViewById(R.id.filter_condition_ivAddNum).setOnClickListener(view -> {
            if (lotteryAdapter.getCount() < 10) {
                ContextHelper.getActivity(SelectP5NumActivity.class).setAddMode();
            } else {
