...
import com.cp2y.cube.adapter.MyFilterP3ZuXuanAdapter;
 ...
 */
public class P3Zu6Fragment extends FilterBaseFragment implements BallSelectCall, MyInterface.setSelectNumD3PopUps, MyInterface.isSavaNumSuccess {
    ...
    private MyFilterP3ZuXuanAdapter adapter;
    ...
    private void initListener() {
        ...
        btn_cancel.setOnClickListener((v -> ContextHelper.getActivity(SelectP3NumActivity.class).gotoConditionPage()));
        btn_back.setOnClickListener((v -> ContextHelper.getActivity(SelectP3NumActivity.class).gotoConditionPage()));
            ...
            if (loginState) {
                if ((list_select_nomal.size() + list_select_dantuo.size()) > 2) {
                    if (list_select_dantuo.size() == 0) {
                        ...
                        if (list.size() == 3) {//单式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 4, 3,selectCount);
                        } else {//复式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 5, 3,selectCount);
                        }
                    } else {//胆拖
                        //判断相同号码
                        list_check2.clear();
                        ...
                        CommonUtil.SortCollection(list);//排序
                        getService(D3LotteryService.class).saveLotteryNumber(list, 6, 3,selectCount);
                    }
                } else {
    ...
    private void initData() {
        ...
        adapter = new MyFilterP3ZuXuanAdapter(getActivity(), false, list_select_nomal, list_select_dantuo);
    ...
    private void numSet(int pos1, int pos2) {
        SelectP3NumActivity act = ContextHelper.getActivity(SelectP3NumActivity.class);
    ...
    public void submit_filter(int type) {
        ...
        if (SingletonMap3D.getMapSize() >= 10 && isEdit == false) {
            ...
            ContextHelper.getActivity(SelectP3NumActivity.class).gotoConditionPage();
        } else {
...
//                Toolbar toolbar=ContextHelper.getActivity(SelectP3NumActivity.class).getToolBar();
//                toolbar.measure(0,0);
