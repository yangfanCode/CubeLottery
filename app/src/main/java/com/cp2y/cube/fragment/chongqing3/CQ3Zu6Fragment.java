package com.cp2y.cube.fragment.chongqing3;
...
import com.cp2y.cube.fragment.chongqing3.adapter.MyFilterCQ3ZuXuanAdapter;
 ...
 */
public class CQ3Zu6Fragment extends FilterBaseFragment implements BallSelectCall, MyInterface.setSelectNumD3PopUps, MyInterface.isSavaNumSuccess {
    ...
    private MyFilterCQ3ZuXuanAdapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_d3_zu6, container, false);
    ...
    private void initListener() {
        ...
        btn_cancel.setOnClickListener((v -> ContextHelper.getActivity(SelectCQ3XingNumActivity.class).gotoConditionPage()));
        btn_back.setOnClickListener((v -> ContextHelper.getActivity(SelectCQ3XingNumActivity.class).gotoConditionPage()));
            ...
            if (loginState) {
                if ((list_select_nomal.size() + list_select_dantuo.size()) > 2) {
                    if (list_select_dantuo.size() == 0) {
                        ...
                        if (list.size() == 3) {//单式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 4, 5,selectCount);
                        } else {//复式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 5, 5,selectCount);
                        }
                    } else {
                        //判断相同号码
                        list_check2.clear();
                        ...
                        CommonUtil.SortCollection(list);//排序
                        getService(D3LotteryService.class).saveLotteryNumber(list, 6, 5,selectCount);
                    }
                }else {
    ...
    private void initData() {
        ...
        adapter = new MyFilterCQ3ZuXuanAdapter(getActivity(), false, list_select_nomal, list_select_dantuo);
    ...
    public void shark() {
        clear();
        List<Integer> list = CombineAlgorithm.sharkRandomNum(3, 10);
    ...
    private void numSet(int pos1, int pos2) {
        SelectCQ3XingNumActivity act = ContextHelper.getActivity(SelectCQ3XingNumActivity.class);
    ...
    public void submit_filter(int type) {
        ...
        if (SingletonMap3D.getMapSize() >= 10 && isEdit == false) {
            ...
            ContextHelper.getActivity(SelectCQ3XingNumActivity.class).gotoConditionPage();
        } else {
    ...
    private void saveNum(int type, ArrayList<String> listfilter_zu6, int click) {
        ...
        SelectCQ3XingNumActivity act = ContextHelper.getActivity(SelectCQ3XingNumActivity.class);
        if (type == 2) {//编辑模式
            SingletonMap3D.editDelete(act.getKey(), listfilter_zu6);
        } else if (type == 1) {//添加号码
            SingletonMap3D.registerService(SingletonMap3D.sign_sort + "_" + click, listfilter_zu6);
            click++;
            ...
            SingletonMap3D.registerService(CommonUtil.cutFirstKey(act.getKey()) + "_" + click, listfilter_zu6);
            click++;
        }
...
//                Toolbar toolbar=ContextHelper.getActivity(SelectCQ3XingNumActivity.class).getToolBar();
//                toolbar.measure(0,0);
