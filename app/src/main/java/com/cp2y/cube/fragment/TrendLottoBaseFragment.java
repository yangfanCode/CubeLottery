 ...
 */
public abstract class TrendLottoBaseFragment extends BaseFragment {
    ...
    public void closePop(){
            LottoTrendPicActivity activity= (LottoTrendPicActivity) getActivity();
            activity.ClosePop();
    }
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (settingDialog == null) {//默认值
            ...
            //获取自定义的VIew
            View trendNumView=LayoutInflater.from(getActivity()).inflate(R.layout.dialog_lotto_numtrend, null);
            //添加自定义VIew
...
//            radioGroup.check(R.id.startId+2);
            ...
            trendNumView.findViewById(R.id.PositiveButton).setOnClickListener(v -> {//点击确定的时候改变筛选条件
                ...
                condition.setWeekDay(MapUtils.LOTTO_TREND_WEEK[weeks.getCheckedRadioButtonId() - R.id.startId]);
           ...
           // RadioGroup radioGroup = (RadioGroup) settingDialog.findViewById(R.id.dialog_numTrend_Count_rg);
            ...
            weeks.check(R.id.startId + MapUtils.TREND_LOTTO_WEEK_MAP.get(condition.getCurrentWeekDay()));
