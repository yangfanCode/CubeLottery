 ...
 */
public class P3ConditionFragment extends Fragment implements HistoryDialog.HistoryCall, DoubleNumDialog.DoubleNumCall, SumDialog.SumCall, SpanDialog.SpanCall, D3BigDialog.BigCall, D3OddDialog.OddCall, DivideDialog.DivideCall,SumMantissaDialog.MantissaCall {
    ...
    private void loadConditions() {
        if (SingletonMapFilter.getP3Map().size() > 0) {
            myFilter_layout.setVisibility(View.VISIBLE);
            for (Iterator<Map.Entry<String, List<String>>> it = SingletonMapFilter.getP3Map().entrySet().iterator(); it.hasNext(); ) {
    ...
    private void initData() {
        ...
        historyDialog.setFlag(3);//setFlag为公用的弹窗
        ...
        listView.setOnItemClickListener((adapterView, view1, i, l) -> {//跳转编辑
            ...
            ContextHelper.getActivity(SelectP3NumActivity.class).editData(list, entry.getKey(), keyNum);
        });
    ...
    private void initListener() {
        ...
        findViewById(R.id.filter_condition_ivAddNum).setOnClickListener(view -> {
            if (lotteryAdapter.getCount() < 10) {
                ContextHelper.getActivity(SelectP3NumActivity.class).setAddMode();
            } else {
