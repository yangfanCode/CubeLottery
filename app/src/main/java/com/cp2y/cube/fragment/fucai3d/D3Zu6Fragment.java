...
import com.cp2y.cube.adapter.MyFilter3DZuXuanAdapter;
 ...
 */
public class D3Zu6Fragment extends FilterBaseFragment implements BallSelectCall, MyInterface.setSelectNumD3PopUps, MyInterface.isSavaNumSuccess {
    ...
    private MyFilter3DZuXuanAdapter adapter;
            ...
            if (loginState) {
                if ((list_select_nomal.size() + list_select_dantuo.size()) > 2) {
                    if (list_select_dantuo.size() == 0) {
                        ...
                        if (list.size() == 3) {//单式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 4, 2,selectCount);
                        } else {//复式
                            getService(D3LotteryService.class).saveLotteryNumber(list, 5, 2,selectCount);
                        }
                    } else {//胆拖
                        //判断相同号码
                        list_check2.clear();
                        ...
                        CommonUtil.SortCollection(list);//排序
                        getService(D3LotteryService.class).saveLotteryNumber(list, 6, 2,selectCount);
                    }
                } else {
    ...
    private void initData() {
        ...
        adapter = new MyFilter3DZuXuanAdapter(getActivity(), false, list_select_nomal, list_select_dantuo);
