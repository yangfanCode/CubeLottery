package com.cp2y.cube.activity.ignore.pailie3;
...
import com.cp2y.cube.activity.ignore.D3BaseActivity;
import com.cp2y.cube.adapter.D3PartConditionIgnoreAdapter;
import com.cp2y.cube.adapter.D3PartIgnoreAdapter;
...
/**
 * 分区形态遗漏activity  展示排列3,奇偶,除3余,大小,号码遗漏
 */
public class PartIgnoreP3Activity extends D3BaseActivity implements MyInterface.IgnoreDateChange {
    ...
    private ListView lv;
    private D3PartIgnoreAdapter adapter;
    private D3PartConditionIgnoreAdapter adapter_condition;
    private HorizontalScrollView Hscrollview;
    private LinearLayout num_trend_layout;
    private String[] bigData = {"大大大", "大大小","大小大","大小小","小大大","小大小","小小大","小小小"};
    private String[] oddData = {"奇奇奇", "奇奇偶","奇偶奇","奇偶偶","偶奇奇","偶奇偶","偶偶奇","偶偶偶"};
    ...
    private List<CheckBox> list_oddPattern_check = new ArrayList<>();//奇偶比形态选择控件
    private List<CheckBox> list_oddNum_check = new ArrayList<>();//奇偶比选择控件
    private List<CheckBox> list_bigPattern_check = new ArrayList<>();//大小比形态选择控件
    ...
    private void initListener() {
        ...
        tv_submit.setOnClickListener((v) -> {
            if(list_divideNum_check!=null&&list_divideNum_check.size()>0&&pos==3&&tabLayout.getSelectedTabPosition()==0){
                ...
                if(list_divide_num!=null&&list_divide_num.size()>0){
                    ...
                    SingletonMapFilter.registerP3Service("10", list_yu);
                }
                ...
                if(list_divide_pattern!=null&&list_divide_pattern.size()>0){
                    ...
                    SingletonMapFilter.registerP3Service("9", list_divide_pattern);
                }
                ...
                if(list_odd_num!=null&&list_odd_num.size()>0) {
                    ...
                    SingletonMapFilter.registerP3Service("8", list_odd_num);
                }
                ...
                if(list_odd_pattern!=null&&list_odd_pattern.size()>0) {
                    ...
                    SingletonMapFilter.registerP3Service("7", list_odd_pattern);
                }
                ...
                if(list_big_num!=null&&list_big_num.size()>0) {
                    ...
                    SingletonMapFilter.registerP3Service("13", list_big_num);
                }
                ...
                if(list_big_pattern!=null&&list_big_pattern.size()>0) {
                    ...
                    SingletonMapFilter.registerP3Service("12", list_big_pattern);
                }
            }
        ...
        }else if(list_bigNum_check!=null&&list_bigNum_check.size()>0&&pos==6){//大小比监听
    ...
    private void reloadIgnoreData() {
        ...
        if (tabLayout.getSelectedTabPosition() == 0) {//个数
            if(pos==0){//号码
                ...
                if (data_condition_count != null && data_condition_count.size() > 0) {
                    ...
                    if (list_divideNum_check != null && list_divideNum_check.size() > 0 && pos == 3) {
                        ...
                        setOddNumListener(list_oddNum_check, tv_submit);
                    } else if (list_bigNum_check != null && list_bigNum_check.size() > 0 && pos == 6) {//奇偶比监听
                        setOddNumListener(list_bigNum_check, tv_submit);
                    }
            ...
            if(pos==0){//号码
                if (data_condition_special != null && data_condition_special.size() > 0) {
                    adapter.loadData(data_condition_special, pos, tabLayout.getSelectedTabPosition());
                } else {
                    AVLoading.setVisibility(View.GONE);
                    TipsToast.showTips("网络请求失败");
                }
                ...
                if (data_condition_pattern != null && data_condition_pattern.size() > 0) {
                    ...
                    if (list_oddPattern_check != null && list_oddPattern_check.size() > 0 && pos == 5) {
                        setOddNumListener(list_oddPattern_check, tv_submit);//奇偶形态监听
                    }
                    if (list_bigPattern_check != null && list_bigPattern_check.size() > 0 && pos == 6) {
                        setOddNumListener(list_bigPattern_check, tv_submit);//奇偶形态监听
                    }
    ...
    private void initData() {
        if (pos == 0) {//号码遗漏
            ...
            adapter = new D3PartIgnoreAdapter(this);
            lv.setAdapter(adapter);
        } else if (pos == 3) {//除3余
            setDivideIgnore();//设置标题
            ignore_title.setText("个数");
            adapter_condition = new D3PartConditionIgnoreAdapter(this);
    ...
    public void setNumIgnore() {
        ...
        tabLayout.addTab(tabLayout.newTab().setText("号码遗漏"));
        tabLayout.addTab(tabLayout.newTab().setText("特殊遗漏"));
    }
    ...
    public void addHscrollview(int pos, boolean type) {
        ...
        if (pos == 3) {//除3余控件初始化
            ...
            view = LayoutInflater.from(this).inflate(type == true ? R.layout.ignore_divide_d3_number_select : R.layout.ignore_divide_d3_pattern_select, null);
            ...
            if(pos==5){
                view = LayoutInflater.from(this).inflate(type == true ?R.layout.ignore_odd_d3_number_select:R.layout.ignore_odd_d3_pattern_select, null);
            }else if(pos==6){
                view = LayoutInflater.from(this).inflate(type == true ?R.layout.ignore_odd_d3_number_select:R.layout.ignore_big_d3_pattern_select, null);
            }
            LinearLayout layout = (LinearLayout) view.findViewById(type == true ? R.id.odd_number_ll:R.id.add_pattern_ll);
            for (int i = 0; i < layout.getChildCount(); i++) {
                if (layout.getChildAt(i) instanceof CheckBox) {
                    if(pos==5){
                        if(type){//奇偶
                            ...
                            list_oddPattern_check.add((CheckBox) layout.getChildAt(i));
                        }
                    }else if(pos==6){//大小
                        if(type){
                            list_bigNum_check.add((CheckBox) layout.getChildAt(i));
                        }else{
                            list_bigPattern_check.add((CheckBox) layout.getChildAt(i));
                        }
                    }
                }
            }
    ...
    private void LoadCountData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10003", typePattern, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_pattern.clear();
            data_condition_pattern = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10003", typeCount, date, targetID);
    ...
    private void LoadPatternData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10003", typeCount, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_count.clear();
            data_condition_count = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10003", typePattern, date, targetID);
        }).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>() {
            @Override
            public void onNext(IgnoreTrendNumModel args) {
                super.onNext(args);
    ...
    private void initOddPattern() {
        ...
        for (int i = 0; i < list_oddPattern_check.size(); i++) {
            if (list_oddPattern_check.get(i).isChecked()) {
                list_odd_pattern.add(oddData[i]);
            } else {
                list_odd_pattern.remove(oddData[i]);
            }
        }
    }
    ...
    private void initBigPattern() {
        ...
        for (int i = 0; i < list_bigPattern_check.size(); i++) {
            if (list_bigPattern_check.get(i).isChecked()) {
                list_big_pattern.add(bigData[i]);
            } else {
                list_big_pattern.remove(bigData[i]);
            }
        }
    }
    ...
    private void LoadNumData(int date) {
        NetHelper.LOTTERY_API.getNumberMissBy3D("10003", date).subscribe(new SafeOnlyNextSubscriber<D3NumberMiss>() {
            @Override
            public void onNext(D3NumberMiss args) {
                ...
                data_condition_special = args.getSpecial();
                adapter.loadData(tabPos == 0 ? data_condition_num : data_condition_special, pos, tabPos);
            }
    ...
    public void sortCondition(boolean type) {//指标排序
        ...
        if (pos == 3) {
            if(tabLayout.getTabAt(0).isSelected()){//个数排序
                ...
                if (type) {//次数占比排序  3次
                    scaleSort(3,data_condition_pattern,1);
                } else {//欲出几率排序
                    emergeSort(3,data_condition_pattern,1);
                }
                adapter_condition.loadData(data_condition_pattern, pos, tabLayout.getSelectedTabPosition());
            }
        } else {
            ...
            if(tabLayout.getTabAt(0).isSelected()){//奇偶个数排序
                ...
                if (type) {//次数占比排序
                    scaleSort(1,data_condition_pattern,1);
                } else {//欲出几率排序
                    emergeSort(1,data_condition_pattern,1);
                }
                adapter_condition.loadData(data_condition_pattern, pos, tabLayout.getSelectedTabPosition());
            }
        }
    ...
    public void sortNum(boolean type) {//号码排序
        ...
        if (tabLayout.getSelectedTabPosition() == 0) {
            ...
            if (type) {//次数占比排序
                if (scale_sort_condition_pattern) {
                    setBiBigSort(data_condition_special,1);
                } else {//倒序
                    setBiSmallSort(data_condition_special, 1);
                }
                scale_sort_condition_pattern = !scale_sort_condition_pattern;
                emerge_sort_condition_pattern = true;
            } else {//欲出几率排序
                if (emerge_sort_condition_pattern) {
                    setEmergeBigSort(data_condition_special, 1);
                } else {//倒序
                    setEmergeSmallSort(data_condition_special, 1);
                }
                emerge_sort_condition_pattern = !emerge_sort_condition_pattern;
                scale_sort_condition_pattern = true;
            }
