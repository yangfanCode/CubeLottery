 ...
 */
public class CalcD3ZuXuanFragment extends Fragment {
...
//                            editText.setText(textTemp);
    ...
    private void numSet(int pos1, int pos2) {
        D3CalcFragment fragment = (D3CalcFragment) CalcD3ZuXuanFragment.this.getParentFragment();
...
//        }
        ...
        if (type == 2) {//编辑模式 可能切换过来编辑很多 所以重新put
            for (int i = 0, size = list_select.size(); i < size; i++) {
                SingletonMap3D.registerService(fragment.getKey(), list_select.get(i));
            }
        } else if (type == 1) {//添加号码
            for (int i = 0, size = list_select.size(); i < size; i++) {
                SingletonMap3D.registerService(SingletonMap3D.sign_sort + "_" + onClickFlag, list_select.get(i));
                onClickFlag++;
...
//            SingletonMap3D.registerService(CommonUtil.cutFirstKey(fragment.getKey()) + "_" + onClickFlag, listfilter_zu6);
//            onClickFlag++;
        }
