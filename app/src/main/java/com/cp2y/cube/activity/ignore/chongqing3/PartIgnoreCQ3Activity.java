package com.cp2y.cube.activity.ignore.chongqing3;
...
/**
 * 分区形态遗漏activity  展示重庆时时彩3星,奇偶,除3余,大小号码遗漏
 */
public class PartIgnoreCQ3Activity extends D3BaseActivity implements MyInterface.IgnoreDateChange {
        ...
        tv_submit.setOnClickListener((v) -> {
            if(list_divideNum_check!=null&&list_divideNum_check.size()>0&&pos==3&&tabLayout.getSelectedTabPosition()==0){
                ...
                if(list_divide_pattern!=null&&list_divide_pattern.size()>0){
                    ...
                    SingletonMapFilter.registerCQ3Service("9", list_divide_pattern);
                }
                ...
                if(list_odd_num!=null&&list_odd_num.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ3Service("8", list_odd_num);
                }
                ...
                if(list_odd_pattern!=null&&list_odd_pattern.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ3Service("7", list_odd_pattern);
                }
                ...
                if(list_big_num!=null&&list_big_num.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ3Service("13", list_big_num);
                }
                ...
                if(list_big_pattern!=null&&list_big_pattern.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ3Service("12", list_big_pattern);
                }
            }
            ...
            if(pos==0){//号码
                ...
                if (data_condition_pattern != null && data_condition_pattern.size() > 0) {
                    ...
                    if (list_bigPattern_check != null && list_bigPattern_check.size() > 0 && pos == 6) {
                        setOddNumListener(list_bigPattern_check, tv_submit);//大小形态监听
                    }
    ...
    private void LoadCountData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10093", typePattern, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_pattern.clear();
            data_condition_pattern = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10093", typeCount, date, targetID);
    ...
    private void LoadPatternData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10093", typeCount, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_count.clear();
            data_condition_count = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10093", typePattern, date, targetID);
        }).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>() {
            @Override
            public void onNext(IgnoreTrendNumModel args) {
                super.onNext(args);
    ...
    private void LoadNumData(int date) {
        NetHelper.LOTTERY_API.getNumberMissBy3D("10093", date).subscribe(new SafeOnlyNextSubscriber<D3NumberMiss>() {
            @Override
