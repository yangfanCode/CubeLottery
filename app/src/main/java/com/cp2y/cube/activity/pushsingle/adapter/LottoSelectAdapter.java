 ...
 */
public class LottoSelectAdapter extends BaseAdapter{
    ...
    public LottoSelectAdapter(Context context) {
    ...
    private void checkedAll(boolean isCheckedAll){
        if(isCheckedAll){//全选
            SingletonSelectSingleCheck.clearLottoData();//清空数据
            for(int i=0,size=list.size();i<size;i++){
                SingletonSelectSingleCheck.registerLottoService(i);
            }
        }else{//全不选
            SingletonSelectSingleCheck.clearLottoData();
        }
        notify.notifyListView();
    }
    //单勾选
    private void checkdSave(int position,boolean isChecked){
        SingletonSelectSingleCheck.removeLottoMap(0);//先删除全选
        if (isChecked) {
            SingletonSelectSingleCheck.registerLottoService(position);
        } else {
            SingletonSelectSingleCheck.removeLottoMap(position);
        }
        if (list.size() - SingletonSelectSingleCheck.getLottoObj().size() <= 2) {
            if (SingletonSelectSingleCheck.getLottoObj().size() == list.size() - 1) {
                SingletonSelectSingleCheck.registerLottoService(0);
            } else if(SingletonSelectSingleCheck.getLottoObj().size() == list.size() - 2){
                SingletonSelectSingleCheck.removeLottoMap(0);
            }
            notify.notifyListView();
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (convertView == null) {
            if (type == 0) {
                ...
                convertView = inflater.inflate(R.layout.item_lotto_selectsinglee_single, null);
            } else if (type == 2) {
                convertView = inflater.inflate(R.layout.item_selectsingle_muti, null);
            } else if (type == 3) {
                convertView = inflater.inflate(R.layout.item_lotto_selectsingle_dan, null);
            }
        }
        ...
        if (type == 0) {
            ...
            check.setChecked(SingletonSelectSingleCheck.isLottoContains(position));//是否选择
            return convertView;
        }
        ...
        check.setChecked(SingletonSelectSingleCheck.isLottoContains(position));//修改选择状态
