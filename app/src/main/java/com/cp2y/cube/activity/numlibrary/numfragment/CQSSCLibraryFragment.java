...
import com.cp2y.cube.activity.numlibrary.adapter.CQSSCNumberAdapter;
 ...
 */
public class CQSSCLibraryFragment extends Fragment {
    private CQSSCNumberAdapter adapter = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //删除号码库
        FileUtils.cleanInternalFiles(new File(FileUtils.D3_PATH));
        ...
        adapter = new CQSSCNumberAdapter(getActivity());
        //添加头布局
        ...
        content_list.setOnItemClickListener((adapterView, view, i, l) -> {
            ...
            if(content_list.getFooterViewsCount()==1&&i==content_list.getLastVisiblePosition())return;//点击尾部局无效
            NewNumberModel.NumberData numberData=adapter.getItem(i-1);
            String fiveNumber = numberData.getFiveNumber();
            if (fiveNumber.contains(";")&&fiveNumber.split(";").length==6) {//跳转下一个页面
    ...
    private void refreshData() {
        currPage = 0;
        NetHelper.LOTTERY_API.getNumberLibrary(LoginSPUtils.getInt("id",0), 10089, 0, ++currPage, 0)
