package com.cp2y.cube.activity.ignore.chongqing5;
...
/**
 * 分区形态遗漏activity  展示重庆时时彩5星,奇偶,除3余,号码遗漏 大小
 */
public class PartIgnoreCQ5Activity extends P5BaseActivity implements MyInterface.IgnoreDateChange {
        ...
        tv_submit.setOnClickListener((v) -> {
            if (list_divideNum_check != null && list_divideNum_check.size() > 0 && pos == 3 && tabLayout.getSelectedTabPosition() == 0) {
                ...
                if (list_divide_num != null && list_divide_num.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("10", list_yu);
                }
                ...
                if (list_divide_pattern != null && list_divide_pattern.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("9", list_divide_pattern);
                }
                ...
                if (list_odd_num != null && list_odd_num.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("8", list_odd_num);
                }
                ...
                if (list_odd_pattern != null && list_odd_pattern.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("7", list_odd_pattern);
                }
                ...
                if(list_big_num!=null&&list_big_num.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("13", list_big_num);
                }
                ...
                if(list_big_pattern!=null&&list_big_pattern.size()>0) {
                    ...
                    SingletonMapFilter.registerCQ5Service("12", list_big_pattern);
                }
            }
    ...
    private void LoadCountData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10089", typePattern, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_pattern.clear();
            data_condition_pattern = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10089", typeCount, date, targetID);
    ...
    private void LoadPatternData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10089", typeCount, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_count.clear();
            data_condition_count = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10089", typePattern, date, targetID);
        }).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>() {
            @Override
            public void onNext(IgnoreTrendNumModel args) {
                super.onNext(args);
    ...
    private void LoadNumData(int date) {
        NetHelper.LOTTERY_API.getNumberMissBy3D("10089", date).subscribe(new SafeOnlyNextSubscriber<D3NumberMiss>() {
            @Override
