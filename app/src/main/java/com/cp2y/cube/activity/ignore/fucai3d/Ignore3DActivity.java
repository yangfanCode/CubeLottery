...
/**
 * 普通不分区形态遗漏activity  展示3D,跨度,和值遗漏 和尾数
 */
public class Ignore3DActivity extends D3BaseActivity implements MyInterface.IgnoreDateChange{
    ...
    private void initListener() {
        tv_submit.setOnClickListener((v) -> {
            if(list_span_check!=null&&list_span_check.size()>0&&pos==2){
                ...
                initSpanData();
                saveD3Span(list_span,list_span_flag,2);
                ...
                initSumData();
                saveD3Sum(list_sum,list_sum_flag,2);
                ...
                initSumMantissaData();
                saveD3SumMantissa(list_sumMantissa,list_sumMantissa_flag,2);
            }
        });
        //两个排序监听
    ...
    private void LoadConditionData(int targetID,int date,int type){
        NetHelper.LOTTERY_API.getConditionMissTrend("10001",type,date,targetID).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>(){
            @Override
    ...
    public void addHscrollview(int pos){
        ...
        View view=null;
            view= LayoutInflater.from(this).inflate((pos==2||pos==7)?R.layout.ignore_d3_span_scroll:R.layout.ignore_d3_sum_scroll,null);
            //初始化获取控件
