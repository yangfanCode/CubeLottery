...
/**
 * 普通不分区形态遗漏activity  展示排列3,跨度,和值遗漏
 */
public class IgnoreP3Activity extends D3BaseActivity implements MyInterface.IgnoreDateChange{
    ...
    private void initListener() {
        tv_submit.setOnClickListener((v) -> {
            if(list_span_check!=null&&list_span_check.size()>0&&pos==2){
                ...
                initSpanData();
                saveD3Span(list_span,list_span_flag,3);
                ...
                initSumData();
                saveD3Sum(list_sum,list_sum_flag,3);
                ...
                initSumMantissaData();
                saveD3SumMantissa(list_sumMantissa,list_sumMantissa_flag,3);
            }
        });
        //两个排序监听
    ...
    private void LoadConditionData(int targetID,int date,int type){
        NetHelper.LOTTERY_API.getConditionMissTrend("10003",type,date,targetID).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>(){
            @Override
