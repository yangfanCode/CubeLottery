...
/**
 * 普通不分区形态遗漏activity  展示重庆时时彩5星,跨度,和值遗漏
 */
public class IgnoreCQ5Activity extends P5BaseActivity implements MyInterface.IgnoreDateChange{
    ...
    private void initListener() {
        tv_submit.setOnClickListener((v) -> {
            if(list_span_check!=null&&list_span_check.size()>0&&pos==2){
                ...
                initSpanData();
                saveP5Span(list_span,list_span_flag,6);
            }else if(list_sumMantissa_check!=null&&list_sumMantissa_check.size()>0&&pos==7){
                //和尾数条件保存
                ...
                initSumMantissaData();
                saveP5SumMantissa(list_sumMantissa,list_sumMantissa_flag,6);
            }else{//和值保存条件
                //跨度条件保存
                ...
                initSumData();
                saveP5Sum(list_sum,list_sum_flag,6);
            }
        });
        //两个排序监听
    ...
    private void LoadConditionData(int targetID,int date,int type){
        NetHelper.LOTTERY_API.getConditionMissTrend("10089",type,date,targetID).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>(){
            @Override
    ...
    public void addHscrollview(int pos){
        ...
        View view=null;
        view= LayoutInflater.from(this).inflate((pos==2||pos==7)?R.layout.ignore_d3_span_scroll:R.layout.ignore_p5_sum_scroll,null);
        //初始化获取控件
        ...
        for(int i=0;i<layout.getChildCount();i++){
            if(layout.getChildAt(i) instanceof CheckBox){
                if(pos==2){
                    ...
                    list_sumMantissa_check.add((CheckBox) layout.getChildAt(i));//和值
                }
            }
        }
        if(view!=null){
