 ...
 */
public class SelectD3Fragment extends Fragment implements MyInterface.partNotify{
    private ListView lv;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView=inflater.inflate(R.layout.fragment_select_d3, container, false);
        initView(rootView);
        initData();
        initLisener();
        return rootView;
    ...
    private void initData() {
        NetHelper.LOTTERY_API.getSelectSingleNumber(LoginSPUtils.getInt("id", 0),10001).subscribe(new SafeOnlyNextSubscriber<SelectSingleNumModel>(){
            @Override
    ...
    private void initView(View rootView) {
        ...
        adapter=new D3SelectAdapter(getActivity(),2);
    ...
    private void updateSingle(int position) {
        ...
        if (position >= firstVisiblePosition && position <= lastVisiblePosition) {
            ...
            //头布局占以为 -1
            if(check!=null)check.setChecked(SingletonSelectSingleCheck.isD3Contains(position-1));//是否选择
        }
    }
    ...
    public String getSelectSingleData() {
        ...
        for (Iterator<Integer> iterator = SingletonSelectSingleCheck.getD3Obj().iterator(); iterator.hasNext();) {
