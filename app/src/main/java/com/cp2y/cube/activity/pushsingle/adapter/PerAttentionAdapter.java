 ...
 */
public class PerAttentionAdapter extends BaseAdapter {
    ...
    private View.OnClickListener onClickListener=v -> {
        int position= (int) v.getTag(R.id.position);
        int userId=list.get(position).getSubscribeUserId();
        EventBus.getDefault().post(new MessageEvent(MessageEvent.PERSONAL_OPENACT,userId));
    };
    public PerAttentionAdapter(Context context){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        button.setVisibility(detail.getSubscribeUserId()== LoginSPUtils.getInt("id", 0)? View.GONE:View.VISIBLE);//自己隐藏
