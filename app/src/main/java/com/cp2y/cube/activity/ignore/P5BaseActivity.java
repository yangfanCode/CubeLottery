...
public class P5BaseActivity extends BaseActivity {
...
//                            }
	...
	//跨度保存条件 排列5 重庆时时彩5星6
	public void saveP5Span(List<String> list_span, List<String> list_span_flag, int flag) {
		if (list_span != null && list_span.size() > 0) {
			list_span_flag.clear();
			list_span_flag.addAll(list_span);
			if (flag == 4) {
				SingletonMapFilter.registerP5Service("4", list_span);
			} else if (flag == 6) {
				SingletonMapFilter.registerCQ5Service("4", list_span);
			}
		}
		TipsToast.showTips("已成功保存至过滤条件");
	}
	//和尾数保存条件 排列5 重庆时时彩5星6
	public void saveP5SumMantissa(List<String> list_sumMantissa, List<String> list_sumMantissa_flag, int flag) {
		if (list_sumMantissa != null && list_sumMantissa.size() > 0) {
			list_sumMantissa_flag.clear();
			list_sumMantissa_flag.addAll(list_sumMantissa);
			if (flag == 4) {
				SingletonMapFilter.registerP5Service("11", list_sumMantissa);
			} else if (flag == 6) {
				SingletonMapFilter.registerCQ5Service("11", list_sumMantissa);
			}
		}
		TipsToast.showTips("已成功保存至过滤条件");
	}
	...
	//和值保存条件 排列5 重庆时时彩5星6
	public void saveP5Sum(List<String> list_sum, List<String> list_sum_flag, int flag) {
		if (list_sum != null && list_sum.size() > 0) {
			list_sum_flag.clear();
			list_sum_flag.addAll(list_sum);
			if (flag == 4) {
				SingletonMapFilter.registerP5Service("1", list_sum);
			} else if (flag == 6) {
				SingletonMapFilter.registerCQ5Service("1", list_sum);
			}
		}
		TipsToast.showTips("已成功保存至过滤条件");
	}
				...
				((RadioButton) radioGroup.getChildAt(i)).setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
						if (isChecked) {
							...
							} else if ((posType == 5 && tab == 1) || (posType == 3 && tab == 1)||(posType == 6 && tab == 1)) {//除3余形态  奇偶形态
