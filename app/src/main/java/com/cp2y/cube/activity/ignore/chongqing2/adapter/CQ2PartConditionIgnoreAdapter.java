package com.cp2y.cube.activity.ignore.chongqing2.adapter;
 ...
 */
public class CQ2PartConditionIgnoreAdapter extends BaseAdapter {
    private int tab = 0, pos = 0;
    private int maxLeft = 0, maxRight = 0;
    private Context context;
    private LayoutInflater inflater;
    private List<IgnoreTrendNumModel.MissData> data_total = new ArrayList<>();
    private List<List<IgnoreTrendNumModel.MissData>> list = new ArrayList<>();

    public CQ2PartConditionIgnoreAdapter(Context context) {
    ...
    public int getItemViewType(int position) {
        ...
        if ((pos == 5) || (pos == 6) || (pos == 3 && tab == 1)) {//正常布局
            //奇偶比
            return 1;
        } else {//分区布局
            //除3余个数
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (type == 0) {//头布局
            ...
            try {
                ...
                if (pos == 5 || pos == 6 || (pos == 6 && tab == 1) || (pos == 3 && tab == 1)) {//奇偶终端线
                    endLine.setVisibility(position == getCount() - 1 ? View.VISIBLE : View.GONE);//终端线
                }
                tv_num.setTextSize(TypedValue.COMPLEX_UNIT_PX, ((pos == 5 && tab == 1) || (pos == 6 && tab == 1)) ? context.getResources().getDimension(R.dimen.app_tvSmall_size) : context.getResources().getDimension(R.dimen.app_tvBall_size));
            ...
            if (position > list.get(0).size() && position <= list.get(0).size() + list.get(1).size() + 1) {
    ...
    public void initDivideData(int pos) {
        if (pos == 3 || pos == 5 || pos == 0 || pos == 6) {//除3余数据 奇偶数据 包括不含蓝球
    ...
    public void initOddData(int pos) {
        if (pos == 5 || pos == 0 || pos == 6 || pos == 3) {//除3余数据
