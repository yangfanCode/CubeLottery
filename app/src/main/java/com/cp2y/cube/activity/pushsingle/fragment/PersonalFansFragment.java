...
import com.cp2y.cube.activity.pushsingle.adapter.PerFansAdapter;
 ...
 * 他人个人中心粉丝页面
 */
public class PersonalFansFragment extends Fragment implements MyInterface.SubscribePersonal {
    private int totalPage = 0, currentPage = 1;
    private ListView lv_child;
    private PullToRefreshListView lv;
    private PerFansAdapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_personal_fans, container, false);
    ...
    public static PersonalFansFragment newInstance(int param1) {
        PersonalFansFragment fragment = new PersonalFansFragment();
    ...
    public void initData() {
        currentPage = 1;
        NetHelper.LOTTERY_API.getPersonalAttentionFansList(personalUserId, personalUserId, 1, currentPage, 10,LoginSPUtils.getInt("id", 0))
    ...
    public void subscribePersonal(int position, int type) {
        ...
        if (CommonUtil.isLogin()) {
            if (type == 0) {//没有互相关注
                NetHelper.LOTTERY_API.doSubscribeUser(LoginSPUtils.getInt("id", 0), adapter.getItem(position).getUserId())
                ...
                dialog.getTvSubmit().setOnClickListener((v -> {
                    alertDialog.dismiss();
                    NetHelper.LOTTERY_API.cancleSubscribeUser(LoginSPUtils.getInt("id", 0), adapter.getItem(position).getUserId(), adapter.getItem(position).getId())
                            .subscribe(new SafeOnlyNextSubscriber<FlagModel>() {
                                @Override
                                public void onNext(FlagModel args) {
                                    super.onNext(args);
