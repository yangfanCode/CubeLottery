 ...
 */
public class CalcD3Zu6Fragment extends Fragment implements BallSelectCall {
    ...
    private void numSet(int pos1, int pos2) {
       D3CalcFragment fragment= (D3CalcFragment) CalcD3Zu6Fragment.this.getParentFragment();
...
//        }
    ...
    private void saveNum(int type, ArrayList<String> listfilter_zu6, int click) {
        ...
        D3CalcFragment fragment= (D3CalcFragment) CalcD3Zu6Fragment.this.getParentFragment();
        if (type == 2) {//编辑模式
            SingletonMap3D.editDelete(fragment.getKey(), listfilter_zu6);
        } else if (type == 1) {//添加号码
            ...
            SingletonMap3D.registerService(CommonUtil.cutFirstKey(fragment.getKey()) + "_" + click, listfilter_zu6);
            click++;
        }
        fragment.gotoConditionPage();
    }
