...
/**
 * 普通不分区形态遗漏activity  展示排列5,跨度,和值遗漏,和尾数
 */
public class IgnoreP5Activity extends P5BaseActivity implements MyInterface.IgnoreDateChange{
    ...
    private void initListener() {
        tv_submit.setOnClickListener((v) -> {
            if(list_span_check!=null&&list_span_check.size()>0&&pos==2){
                ...
                initSpanData();
                saveP5Span(list_span,list_span_flag,4);
                ...
                initSumMantissaData();
                saveP5SumMantissa(list_sumMantissa,list_sumMantissa_flag,4);
            }else{//和值保存条件
                //跨度条件保存
                ...
                initSumData();
                saveP5Sum(list_sum,list_sum_flag,4);
            }
        });
        //两个排序监听
    ...
    private void LoadConditionData(int targetID,int date,int type){
        NetHelper.LOTTERY_API.getConditionMissTrend("10004",type,date,targetID).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>(){
            @Override
