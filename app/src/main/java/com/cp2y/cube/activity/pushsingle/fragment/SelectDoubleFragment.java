...
import com.cp2y.cube.activity.pushsingle.adapter.DoubleSelectAdapter;
 ...
 */
public class SelectDoubleFragment extends Fragment implements MyInterface.partNotify {
    private ListView lv;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_select_double, container, false);
        initView(rootView);
        initData();
        initLisener();
        return rootView;
    ...
    private void initLisener() {
        lv.setOnItemClickListener(((parent, view, position, id) -> {
            ...
            if (lv.getFooterViewsCount() == 1 && position == lv.getLastVisiblePosition())
                return;//点击尾部局无效
    ...
    private void initData() {
        NetHelper.LOTTERY_API.getSelectSingleNumber(LoginSPUtils.getInt("id", 0), 10002).subscribe(new SafeOnlyNextSubscriber<SelectSingleNumModel>() {
            @Override
    ...
    private void initView(View rootView) {
        ...
        adapter = new DoubleSelectAdapter(getActivity());
    ...
    private void updateSingle(int position) {
        ...
        if (position >= firstVisiblePosition && position <= lastVisiblePosition) {
            ...
            //头布局占以为 -1
            if (check != null)
                check.setChecked(SingletonSelectSingleCheck.isDoubleContains(position - 1));//是否选择
        }
    }
    ...
    public String getSelectSingleData() {
        ...
        for (Iterator<Integer> iterator = SingletonSelectSingleCheck.getDoubleObj().iterator(); iterator.hasNext();) {
