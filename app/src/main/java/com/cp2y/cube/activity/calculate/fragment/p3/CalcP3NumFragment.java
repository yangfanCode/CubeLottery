...
import com.cp2y.cube.activity.calculate.fragment.p3.adapter.CalcP3LotteryAdapter;
 ...
 */
public class CalcP3NumFragment extends Fragment implements MyInterface.SetCalcIssue{

    private CalcP3LotteryAdapter lotteryAdapter;
    ...
    private void initListener() {
        tvAdd.setOnClickListener((v -> {
            P3CalcFragment fragment = (P3CalcFragment) CalcP3NumFragment.this.getParentFragment();
            fragment.setAddMode();
        }));
                ...
                subscriber.onNext(file);
                    ...
                    .subscribe(new SafeOnlyNextSubscriber<Object>() {
                        ...
                        public void onNext(Object args) {
                            super.onNext(args);
                            NetHelper.LOTTERY_API.calcLottery((File)args, 10003, issue, 1, false, true)
    ...
    private void initData() {
        lotteryAdapter = new CalcP3LotteryAdapter();
        ...
        listView.setOnMenuItemClickListener((position, menu, index) -> {//删除
            ...
            SingletonMapP3.removeMap(entry.getKey());
            lotteryAdapter.notifyDataSetChanged();
            return false;
        });
    ...
    public void setCalcIssue(String issue) {
        if(TextUtils.isEmpty(issue)){
            tvIssue.setText("(排列3)".concat(issue));
        } else {
            this.issue = issue;
            tvIssue.setText("(排列3 ".concat(issue).concat("期)"));
        }
    }
}
