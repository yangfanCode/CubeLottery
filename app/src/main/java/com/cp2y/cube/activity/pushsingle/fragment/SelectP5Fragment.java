...
import com.cp2y.cube.activity.pushsingle.adapter.P5SelectAdapter;
 ...
 */
public class SelectP5Fragment extends Fragment implements MyInterface.partNotify{
    private ListView lv;
    private AVLoadingIndicatorView AVLoading;
    private RelativeLayout num_library_layout;
    private P5SelectAdapter adapter;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View rootView=inflater.inflate(R.layout.fragment_select_p5, container, false);
        initView(rootView);
        initData();
        initLisener();
        return rootView;
    ...
    private void initData() {
        NetHelper.LOTTERY_API.getSelectSingleNumber(LoginSPUtils.getInt("id", 0),10004).subscribe(new SafeOnlyNextSubscriber<SelectSingleNumModel>(){
            @Override
    ...
    private void initView(View rootView) {
        ...
        adapter=new P5SelectAdapter(getActivity());
    ...
    private void updateSingle(int position) {
        ...
        if (position >= firstVisiblePosition && position <= lastVisiblePosition) {
            ...
            //头布局占以为 -1
            if(check!=null)check.setChecked(SingletonSelectSingleCheck.isP5Contains(position-1));//是否选择
        }
    }
    ...
    public String getSelectSingleData() {
        ...
        for (Iterator<Integer> iterator = SingletonSelectSingleCheck.getP5Obj().iterator(); iterator.hasNext();) {
