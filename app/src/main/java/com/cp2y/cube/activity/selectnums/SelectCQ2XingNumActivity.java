...
import com.cp2y.cube.fragment.chongqing2.CQ2ConditionFragment;
import com.cp2y.cube.fragment.chongqing2.CQ2ZhiXuanFragment;
import com.cp2y.cube.fragment.chongqing2.CQ2ZuXuanFragment;
...
public class SelectCQ2XingNumActivity extends BaseActivity implements SensorEventListener {
    private int tabSelectPos = 0;
    private List<Fragment> list = new ArrayList<>();
    private Toolbar conditionBar, normalBar;
    private TabLayout tablayout;
    private FragmentManager fm = getSupportFragmentManager();
    private CQ2ConditionFragment conditionFragment = new CQ2ConditionFragment();
    ...
    private void initListener() {
        tablayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                if(SingletonMap3D.getMap().size()>0){//有号码数据
                    if((tabSelectPos==0&&position==1)||(tabSelectPos==1&&position==0)){
                        switchDialog(position);//弹窗删除号码
                    }else{
                        switchFragment(position);//取消操作
                    }
                }else{
        ...
        ignore.setOnClickListener(view -> {
            ...
            isIgnore = !isIgnore;
            CQ2ZhiXuanFragment fragment0 = (CQ2ZhiXuanFragment) list.get(0);
            fragment0.switchIgnore();
            CQ2ZuXuanFragment fragment1 = (CQ2ZuXuanFragment) list.get(1);
    ...
    private void switchDialog(int position) {
        AlertDialog.Builder builder = new AlertDialog.Builder(SelectCQ2XingNumActivity.this);
        View view = LayoutInflater.from(SelectCQ2XingNumActivity.this).inflate(R.layout.selectnum_3d_tabswitch, null);
    ...
    private void initData() {
        //遗漏接口数据
        NetHelper.LOTTERY_API.getD3CurrentLotteryMiss("10095")
        ...
        CQ2ZhiXuanFragment zhixuanFragment = new CQ2ZhiXuanFragment();
        CQ2ZuXuanFragment zuxuanFragment = new CQ2ZuXuanFragment();
        list.add(zhixuanFragment);
        list.add(zuxuanFragment);
        fm.beginTransaction().add(R.id.selectNum_filter_container, zhixuanFragment).add(R.id.selectNum_filter_container, zuxuanFragment)
                .add(R.id.selectNum_filter_container, conditionFragment).hide(zuxuanFragment).hide(conditionFragment).commit();
    }
    ...
    private void initView() {
        ...
        tablayout.addTab(tablayout.newTab().setText("组选"));
    }

    //控制fragment
    public void selectPos(int pos1, int pos2) {
        fm.beginTransaction().show(list.get(pos1)).hide(list.get(pos2)).commit();
    }
    ...
    public void editData(List<String>list, String key, int keyNum){
        onBackPressed();
        CQ2ZhiXuanFragment fragment0 = (CQ2ZhiXuanFragment) this.list.get(0);
        CQ2ZuXuanFragment fragment1 = (CQ2ZuXuanFragment) this.list.get(1);
        ...
        if (keyNum < 1000) {//普通模式
            ...
            fragment0.ShowTip();
        } else if(keyNum>3000&&keyNum<6000){//组选
    ...
    protected void onDestroy() {
        ...
        //销毁清空数据 初始化
        SingletonMap3D.getMap().clear();
        SingletonMapFilter.getCQ2Map().clear();
