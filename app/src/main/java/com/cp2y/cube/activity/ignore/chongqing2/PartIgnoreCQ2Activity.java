...
import com.cp2y.cube.activity.ignore.chongqing2.adapter.CQ2PartConditionIgnoreAdapter;
import com.cp2y.cube.activity.ignore.chongqing2.adapter.CQ2PartIgnoreAdapter;
...
/**
 * 分区形态遗漏activity  展示重庆时时彩2星,奇偶,除3余,大小号码遗漏
 */
public class PartIgnoreCQ2Activity extends CQ2BaseActivity implements MyInterface.IgnoreDateChange {
    ...
    private ListView lv;
    private CQ2PartIgnoreAdapter adapter;
    private CQ2PartConditionIgnoreAdapter adapter_condition;
    private HorizontalScrollView Hscrollview;
    private LinearLayout num_trend_layout;
    private String[] bigData = {"大大", "大小", "小大", "小小"};
    private String[] oddData = {"奇奇", "奇偶", "偶奇", "偶偶"};
    private String[] oddNumData = {"2:0", "1:1", "0:2"};
        ...
        tv_submit.setOnClickListener((v) -> {
            if (list_divideNum_check != null && list_divideNum_check.size() > 0 && pos == 3 && tabLayout.getSelectedTabPosition() == 0) {
                ...
                if (list_divide_pattern != null && list_divide_pattern.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ2Service("9", list_divide_pattern);
                }
                ...
                if (list_odd_num != null && list_odd_num.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ2Service("8", list_odd_num);
                }
                ...
                if (list_odd_pattern != null && list_odd_pattern.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ2Service("7", list_odd_pattern);
                }
                ...
                if (list_big_num != null && list_big_num.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ2Service("13", list_big_num);
                }
                ...
                if (list_big_pattern != null && list_big_pattern.size() > 0) {
                    ...
                    SingletonMapFilter.registerCQ2Service("12", list_big_pattern);
                }
            }
    ...
    private void initData() {
        if (pos == 0) {//号码遗漏
            ...
            adapter = new CQ2PartIgnoreAdapter(this);
            lv.setAdapter(adapter);
        } else if (pos == 3) {//除3余
            setDivideIgnore();//设置标题
            ignore_title.setText("个数");
            adapter_condition = new CQ2PartConditionIgnoreAdapter(this);
    ...
    public void addHscrollview(int pos, boolean type) {
        ...
        if (pos == 3) {//除3余控件初始化
            ...
            view = LayoutInflater.from(this).inflate(type == true ? R.layout.ignore_divide_cq2_number_select : R.layout.ignore_divide_cq2_pattern_select, null);
            ...
            if (pos == 5) {
                view = LayoutInflater.from(this).inflate(type == true ? R.layout.ignore_odd_cq2_number_select : R.layout.ignore_odd_cq2_pattern_select, null);
            } else if (pos == 6) {
                view = LayoutInflater.from(this).inflate(type == true ? R.layout.ignore_odd_cq2_number_select : R.layout.ignore_big_cq2_pattern_select, null);
            }
    ...
    private void LoadCountData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10095", typePattern, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_pattern.clear();
            data_condition_pattern = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10095", typeCount, date, targetID);
    ...
    private void LoadPatternData(int targetID, int date, int typeCount, int typePattern) {
        NetHelper.LOTTERY_API.getConditionMissTrend("10095", typeCount, date, targetID).flatMap(IgnoreTrendNumModel -> {
            data_condition_count.clear();
            data_condition_count = IgnoreTrendNumModel.getMiss();
            return NetHelper.LOTTERY_API.getConditionMissTrend("10095", typePattern, date, targetID);
        }).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>() {
            @Override
            public void onNext(IgnoreTrendNumModel args) {
                super.onNext(args);
    ...
    private void initDividePattern() {
        ...
        for (int j = 0; j < list_dividePattern_check.get(0).getChildCount(); j++) {
            View child = list_dividePattern_check.get(0).getChildAt(j);
            if (child instanceof RadioButton) {
                //如果选择,更改数据
                RadioButton button = (RadioButton) child;
                if (button.isChecked()) {
                    String text = button.getText().toString();
                    list_divide_pattern.set(0, text.substring(0, 1));
                    list_divide_pattern.set(1, text.substring(1, text.length()));
                }
            }
        }
    }
    ...
    private void LoadNumData(int date) {
        NetHelper.LOTTERY_API.getNumberMissBy3D("10095", date).subscribe(new SafeOnlyNextSubscriber<D3NumberMiss>() {
            @Override
    ...
    public void sortNum(boolean type) {//号码排序
        ...
        if (type) {//次数占比排序  2次
            if (scale_sort_condition_num) {
                setBiBigSort(data_condition_num, 2);
            } else {//倒序
                setBiSmallSort(data_condition_num, 2);
            }
            scale_sort_condition_num = !scale_sort_condition_num;
            emerge_sort_condition_num = true;
        } else {//欲出几率排序
            if (emerge_sort_condition_num) {
                setEmergeBigSort(data_condition_num, 2);
            } else {//倒序
                setEmergeSmallSort(data_condition_num, 2);
            }
            emerge_sort_condition_num = !emerge_sort_condition_num;
            scale_sort_condition_num = true;
        }
