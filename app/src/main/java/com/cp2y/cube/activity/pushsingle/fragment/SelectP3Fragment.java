...
import com.cp2y.cube.activity.pushsingle.adapter.D3SelectAdapter;
 ...
 */
public class SelectP3Fragment extends Fragment implements MyInterface.partNotify{
    private ListView lv;
    private AVLoadingIndicatorView AVLoading;
    private RelativeLayout num_library_layout;
    private D3SelectAdapter adapter;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View rootView=inflater.inflate(R.layout.fragment_select_p3, container, false);
        initView(rootView);
        initData();
        initLisener();
        return rootView;
    ...
    private void initData() {
        NetHelper.LOTTERY_API.getSelectSingleNumber(LoginSPUtils.getInt("id", 0),10003).subscribe(new SafeOnlyNextSubscriber<SelectSingleNumModel>(){
            @Override
    ...
    private void initView(View rootView) {
        ...
        adapter=new D3SelectAdapter(getActivity(),3);
    ...
    private void updateSingle(int position) {
        ...
        if (position >= firstVisiblePosition && position <= lastVisiblePosition) {
            ...
            //头布局占以为 -1
            if(check!=null)check.setChecked(SingletonSelectSingleCheck.isP3Contains(position-1));//是否选择
        }
    }
    ...
    public String getSelectSingleData() {
        ...
        for (Iterator<Integer> iterator = SingletonSelectSingleCheck.getP3Obj().iterator(); iterator.hasNext();) {
