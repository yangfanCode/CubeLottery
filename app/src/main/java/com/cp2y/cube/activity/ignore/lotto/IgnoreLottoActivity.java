 ...
 */
public class IgnoreLottoActivity extends DoubleBaseActivity implements MyInterface.IgnoreDateChange,MyInterface.IgnoreIsBlue{
    ...
    private int pos=-1;
    private TextView title,tv_submit,selectDate,ignore_title,select_red,select_blue;
    ...
    private List<CheckBox>list_lotto_num_check=new ArrayList<>();
    private List<CheckBox>list_lotto_span_check=new ArrayList<>();
    private List<CheckBox>list_lotto_sumMantissa_check=new ArrayList<>();
    ...
    private void initListener() {
        ...
        tv_submit.setOnClickListener((v) -> {
            if(list_lotto_num_check!=null&&list_lotto_num_check.size()>0&&pos==0){
                boolean loginState = LoginSPUtils.isLogin();
                if (loginState) {
                    initSubmitData();
                    saveNum(list_red,list_blue,1);//大乐透基本号保存
                }else{
                    intentLogin();//登录页面
                }
            }else if(list_lotto_span_check!=null&&list_lotto_span_check.size()>0&&pos==2){
                //跨度条件保存
                ...
                initSpanData();
                saveSpan(list_span,list_span_flag,1);
            }else if(list_lotto_sumMantissa_check!=null&&list_lotto_sumMantissa_check.size()>0&&pos==7){
                //和尾数条件保存
                ...
                initSumMantissaData();
                saveSumMantissan(list_sumMantissa,list_sumMantissa_flag,1);
            }else{//和值保存条件
                ...
                if(!TextUtils.isEmpty(sum)){
                    ...
                    SingletonMapFilter.registerLottoService(1 + "", list_sum);
        ...
        //选大乐透基本号监听
        if(list_lotto_num_check!=null&&list_lotto_num_check.size()>0&&pos==0){
            setLottoCheckListener(list_lotto_num_check,tv_submit,num_trend_layout,adapter_red,adapter_blue,list_red_select,list_blue_select);
        }else if(list_lotto_span_check!=null&&list_lotto_span_check.size()>0&&pos==2){//跨度选择条件监听
            //跨度选择监听
            setSpanCheckListener(list_lotto_span_check,tv_submit);
        }else if(sum_rg!=null){//和值选择监听
            setSumSelectListener(sum_rg,tv_submit);
        }else if(list_lotto_sumMantissa_check!=null&&list_lotto_sumMantissa_check.size()>0&&pos==7){//和尾数选择条件监听
            //和尾数选择监听
            setSpanCheckListener(list_lotto_sumMantissa_check,tv_submit);
        }
    }
    //tablayout切换重新加载 红篮球
    ...
    private void initView() {
        ...
        select_red = (TextView) findViewById(R.id.num_trend_tvRedSelect);//选号提示
        select_blue = (TextView) findViewById(R.id.num_trend_tvBlueSelect);//选号提示
        select_red.setText("前");
        select_blue.setText("后");
        ...
        data_red_num=new ArrayList<>();//接收接口数据
    ...
    public void setRedBlueIgnore(){
        ...
        tabLayout.addTab(tabLayout.newTab().setText("前区遗漏"));
        tabLayout.addTab(tabLayout.newTab().setText("后区遗漏"));
    }
    //设置普通标题
    ...
    public void LoadRedData(int type,int date){
        //请求网络 下载红篮球数据
        NetHelper.LOTTERY_API.getMissTrend("10088",type,date).subscribe(new SafeOnlyNextSubscriber<IgnoreTrendNumModel>(){
            @Override
    ...
    public void addHscrollview(int pos){
        ...
        View view=null;
        if(pos==4){//和值
            view=LayoutInflater.from(this).inflate(R.layout.ignore_lotto_sum_scroll,null);
            sum_rg= (RadioGroup) view.findViewById(R.id.sum_rg);
        }else{//跨度和基本号
            view= LayoutInflater.from(this).inflate(pos==0?R.layout.ignore_lotto_selectnum_scroll:(pos==2?R.layout.ignore_lotto_span_scroll:R.layout.ignore_d3_span_scroll),null);
            //初始化获取控件
            ...
            for(int i=0;i<layout.getChildCount();i++){
                if(layout.getChildAt(i) instanceof CheckBox){
                    if(pos==0){
                        list_lotto_num_check.add((CheckBox) layout.getChildAt(i));//基本号
                    }else if(pos==2){
                        list_lotto_span_check.add((CheckBox) layout.getChildAt(i));//跨度
                    }else{
                        list_lotto_sumMantissa_check.add((CheckBox) layout.getChildAt(i));//和尾数
                    }
                }
            }
        }
    ...
    public void initSubmitData(){
        list_red.clear();
        list_blue.clear();
        for (int i = 0; i < list_lotto_num_check.size(); i++) {
            if (i < 35) {
                if (list_lotto_num_check.get(i).isChecked()) {
                    list_red.add((i + 1) + "");
                }
            } else {
                if (list_lotto_num_check.get(i).isChecked()) {
    ...
    public void initSpanData(){
        list_span.clear();
        for (int i = 0; i < list_lotto_span_check.size(); i++) {
            if(list_lotto_span_check.get(i).isChecked()){
                list_span.add(i+6+"");
            }
        }
    }
    ...
    public void initSumMantissaData(){
        ...
        for (int i = 0; i < list_lotto_sumMantissa_check.size(); i++) {
            if(list_lotto_sumMantissa_check.get(i).isChecked()){
