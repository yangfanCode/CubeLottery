 ...
 */
public class MyFilterLottoNomaiRedBallAdapter extends MyBaseAdapter<String> {
    private float DownX=0,DownY=0;//移动距离
    private Set<String> list_select;
    ...
    private View.OnClickListener listener = view -> {
        ...
        if (call != null) call.onBallSelected(MyFilterLottoNomaiRedBallAdapter.this, val, selected);
    };
    private View.OnTouchListener touchListener=(v, event) -> {//滑动事件
        ...
        switch(event.getAction()){
            case MotionEvent.ACTION_DOWN://显示图片  按下
                ...
                break;
            case MotionEvent.ACTION_MOVE://滑动超过范围图片消失  按下滑动
    ...
    public MyFilterLottoNomaiRedBallAdapter(Context context, int resId, boolean ignore, Set<String>list_select) {
        super(context, resId);
        this.ignore=ignore;
