 ...
 */
public class MyFilterDanTuoRedTuoBallAdapter extends MyBaseAdapter<String> {
    ...
    private SparseIntArray ignores = ContextHelper.getActivity(SelectNumFilterActivity.class).getIgnoreReds();
    private View.OnClickListener listener = view -> {
        int position = (int) view.getTag(R.id.key);
        String val = list.get(position);
        boolean isValid = true;
        if (call != null) isValid = call.onBeforeSelected(MyFilterDanTuoRedTuoBallAdapter.this, val,false);
        if (!isValid) return;
        ...
        if (call != null) call.onBallSelected(MyFilterDanTuoRedTuoBallAdapter.this, val, selected);
    };
    ...
    public MyFilterDanTuoRedTuoBallAdapter(Context context, int resId, boolean ignore, Set<String>list_select_red) {
        super(context, resId);
        this.ignore = ignore;
