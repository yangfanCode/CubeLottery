 ...
 */
public class MyFilterLottoDanTuoRedTuoBallAdapter extends MyBaseAdapter<String> {
    private float DownX=0,DownY=0;//移动距离
    private Set<String>list_select_red = new HashSet<>();
    private Set<String>list_select_red2 = new HashSet<>();
    private boolean ignore = false;//是否显示遗漏
    private BallSelectCall call;
    private MyInterface.setSelectNumD3PopUps popUps;
    private SparseIntArray ignores = ContextHelper.getActivity(LottoSelectNumActivity.class).getIgnoreReds();
    private View.OnClickListener listener = view -> {
        int position = (int) view.getTag(R.id.key);
        String val = list.get(position);
        boolean isValid = true;
        if (call != null) isValid = call.onBeforeSelected(MyFilterLottoDanTuoRedTuoBallAdapter.this, val,false);
        if (!isValid) return;
        ...
        if (call != null) call.onBallSelected(MyFilterLottoDanTuoRedTuoBallAdapter.this, val, selected);
    };
    ...
    public MyFilterLottoDanTuoRedTuoBallAdapter(Context context, int resId, boolean ignore, Set<String>list_select_red) {
        super(context, resId);
        this.ignore = ignore;
        this.list_select_red = list_select_red;
    }

    public void setDanList(Set<String>list_select_red2) {
        this.list_select_red2 = list_select_red2;
    }
    ...
    public void bindData(ViewHolder holder, int position) {
        ...
        //编辑数据展示
        if(list_select_red.contains(val)){
            tv.setBackgroundResource(R.drawable.lottery_ball_red_big_all);
            tv.setTextColor(ColorUtils.WHITE);
        } else if (list_select_red2.contains(val)){
