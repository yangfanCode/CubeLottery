...
public class SingletonMap3D {
     ...
     * @return
     */
    public static int[] findZhiXuanLocationIndex(List<String> list) {
        ...
        for (String num:list)
        {
            if (Integer.valueOf(num) >= 50&&j == 0) {
                j = i;//十位位置
            } else if (Integer.valueOf(num) >= 100 ) {
                break;
            }
            i++;
        }
     ...
     * @return
     */
    public static int findCQ2ZhiXuanLocationIndex(List<String> list) {
    ...
    public static int calcLotteryCount() {
        int totalCount = 0;
        for (Iterator<Map.Entry<String, List<String>>> it = SingletonMap3D.getMap().entrySet().iterator(); it.hasNext(); ) {//优先计算实际彩票数量
