...
public class DragGridView extends GridView {
    ...
    //用来处理是否为长按的Runnable  长按处理
    private Runnable mLongClickRunnable = new Runnable() {

        @Override
        public void run() {
            if(!isDrag){//第一次长按之后震动一次 改变拖拽时间,以后不走此方法
                mDragAdapter.setHideItem(mDragPosition);//刷新异步,所以第一次先隐藏item
                mDragAdapter.setIsDelete();//编辑模式
                mVibrator.vibrate(50); //震动一下
            }
            mStartDragItemView.setVisibility(View.INVISIBLE);//除第一次外隐藏该item
            //根据我们按下的点显示item镜像
    ...
    public void setIsShowBitMap(boolean isShowBitMap){
